...
def calculate_fall_angle(length, width, height, square, touch_square, coeff):
    ...
    """Функция берет высоту центра масс и делет ее на ширину или длину конструкции (в зависимости от того, что больше) конструкции. После этого умножает на коэффицент соприкосновения (соотношение плошади блока, которая соприкосается с нижними блоками ко всей площади блока) и на коэффицент отклонения (находится из положения координат x,y центра масс и средней точки опоры) """
